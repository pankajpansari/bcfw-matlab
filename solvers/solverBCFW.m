...
%        end
	...
	%fprintf('loss_i - model.w = %f \n', loss_i - model.w'*psi_i)
...
%	end
       ...
       % 9) Optionally, update the weighted average:
        ...
        if (options.debug && k >= debug_iter)
            ...
            train_error = average_loss(param, param.predictFn, model_debug);
...
%	    saveas(f, 'intermediate_dual_plot.png');
  ...
  %          fprintf('Duality gap check: gap = %g at pass %d (iteration %d) at time = %g\n', gap, p, k, toc)
        end
